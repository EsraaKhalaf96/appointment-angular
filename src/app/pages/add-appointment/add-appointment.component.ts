...
export class AddAppointmentComponent extends FormBase implements OnInit {
  ...
  ngOnInit() {
    ...
    this.ar = {
      firstDayOfWeek: 0,
      dayNames: ["الأحد", "الإثنين", "الثلاثاء", "الآربعاء", "الخميس", "الجمعة", "السبت"],
      dayNamesShort: ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"],
      dayNamesMin: ["ح", "ن", "ث", "ر", "خ", "ج", "س"],
      monthNames: ["يناير", "فبراير", "مارس", "إبريل", "مايو", "يونيو", "يوليو", "أغسطس", "سبتمبر", "اكتوبر", "نوفمبر", "ديسمبر"],
  ...
  getAppointmentInfoEdit() {
    ...
    this._addAppointment.getAppointmentData(this.referenceID).subscribe(result => {
      if (result.responseType == "1") {
        ...
        this.formObject.controls['customerName'].setValue(result.data[0].customerName);
        this.formObject.controls['customerEmail'].setValue(result.data[0].customerEmail);
        this.formObject.controls['customerMobileNo'].setValue(result.data[0].customerMobileNo);


        this.formObject.controls["serviceId"].setValue(result.data[0].serviceId);
        this.formObject.controls["centerId"].setValue(result.data[0].centerId);
        this.formObject.controls["vehicleClassId"].setValue(result.data[0].vehicleClassId);
        ...
        this.vehicleClassList.push({ label: result.data[0].vehicleClassDescAr, value: result.data[0].vehicleClassId });
        this.serviceTypeList.push({ label: result.data[0].serviceNameAr, value: result.data[0].serviceId });
        this.appointmentCentersList.push({ label: result.data[0].centerNameAr, value: result.data[0].centerId });

        let newdate = result.data[0].apointmentDayDate.split("/").reverse().join("-");
        this.formObject.controls["apointmentDayDate"].setValue(new DatePipe('en-EN').transform(newdate, 'MM/dd/yyyy'));
        ...
        let timeEvent = result.data[0].laneId + "$" + result.data[0].apointmentStartTime + "$" + result.data[0].apointmentEndTime;
        this.timeSlotChange(timeEvent);

        this.timeSlotList = []
        this.timeSlotList.push({ label: result.data[0].startTimeOnly, value: result.data[0].laneId + "$" + result.data[0].apointmentStartTime + "$" + result.data[0].apointmentEndTime });
    ...
    this._addAppointment.getVehicleClasses().subscribe(result => {
      ...
      if (result.data.length > 0) {
        ...
        for (let i = 0; i < result.data.length; i++) {
          this.vehicleClassList.push({ label: result.data[i].descriptionAr, value: result.data[i].id });
        }
      }
  ...
  getServiceTypesList(vclClassId) {
    ...
    this._addAppointment.getServiceTypes(vclClassId).subscribe(result => {
      ...
      if (result.data.length > 0) {
        ...
        for (let i = 0; i < result.data.length; i++) {
          this.serviceTypeList.push({ label: result.data[i].nameAr, value: result.data[i].id });
        }
      }
  ...
  getAppointmentCenterList(type) {
    ...
    this._addAppointment.getAppointmentCenters(this.selectedVehicleClass, this.selectedServiceType).subscribe(result => {
      this.appointmentCentersList = []
      if (result.data.length > 0) {
        this.appointmentCentersList.push({ label: 'اختر موقع مركز الفحص', value: -1 });
        for (let i = 0; i < result.data.length; i++) {
          this.appointmentCentersList.push({ label: result.data[i].nameAr, value: result.data[i].id });
        }
      }
      this.centersDDLDisabled = false;
  ...
  getTimeSlots(date) {
    ...
    this._addAppointment.getTimeSlots(formData).subscribe(result => {
      this.timeSlotList = []
      if (result.data.length > 0) {
        this.timeSlotList.push({ label: 'اختر ميعاد الفحص', value: -1 });
        for (let i = 0; i < result.data.length; i++) {
          this.timeSlotList.push({ label: result.data[i].startTimeOnly, value: result.data[i].laneId + "$" + result.data[i].startTime + "$" + result.data[i].endTime });
        }
      }
